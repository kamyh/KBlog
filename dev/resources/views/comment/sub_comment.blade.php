    ...
    @if(!$comment->isPrincipal())
        <div class="jumbotron">
            ...
            <div class="btn-group btn-group-xs reply" role="group">
                <button onclick="comment('a_{{$comment->id}}')" type="button" class="btn-clean">{{ trans('interface.reply') }}</button>
            </div>
            <p>{{$comment->text}}</p>
            ...
            @foreach($comment->getSubComments() as $sub)
                <div class="jumbotron sub">
                    ...
                    <div class="btn-group btn-group-xs reply" role="group">
                        <button onclick="comment('b_{{$sub->id}}')" type="button" class="btn-clean">{{ trans('interface.reply') }}</button>
                    </div>
                    <p>{{$sub->text}}</p>
